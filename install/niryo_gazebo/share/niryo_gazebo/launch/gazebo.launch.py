...
from launch_ros.parameter_descriptions import ParameterValue          # ← FIX 1
...
def generate_launch_description():
    ...
    # FIX 1: wrap Command() in ParameterValue so ROS 2 knows it's a string
    ...
    # FIX 2: correct topic name  (/wrist_camera/image_raw, not /camera/image_raw)
    # FIX 3: use ] (bidirectional) so the bridge forwards Gazebo → ROS
    camera_bridge = Node(
        package='ros_gz_bridge',
        executable='parameter_bridge',
        arguments=[
    ...
    # Vision + decision delayed 8 s
    delayed_pipeline = TimerAction(
        period=8.0,
        actions=[vision_node],  # decision_node temporarily disabled
    )
